...
aws ec2 run-instances --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=${S1_NAME}}]" --image-id ${S1_IMAGE_ID} --count 1 --instance-type t2.micro --key-name ${S1_KEY_NAME} --security-group-ids ${S1_SG_IDs} --subnet-id ${S1_SUBNET} &> /dev/null
...
echo "Creating '${S2_NAME}'..."
aws ec2 run-instances --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=${S2_NAME}}]" --image-id ${S2_IMAGE_ID} --count 1 --instance-type t2.micro --key-name ${S2_KEY_NAME} --security-group-ids ${S2_SG_IDs} --subnet-id ${S2_SUBNET} &> /dev/null

if [[ $? -eq 0 ]]
then
	echo -e "${GREEN}Created '${S2_NAME}'${NC}"
else
	echo -e "${RED}Something went wrong creating '${S2_NAME}'${NC}"
fi
